...
import { Download, Edit2, Plus, Settings, Trash2, Upload, User, Users } from "lucide-react"
...
export function GlobalTeachersDialog() {
  ...
  const handleExportTeachersExcel = () => {
    ...
    const templateSheet = XLSX.utils.aoa_to_sheet([
      ["اسم المعلم", "رقم الهوية", "رقم الحساب", "رقم الجوال", "الحلقة", "المسمى"],
      ["", "", "", "", "", "teacher"],
    ])

    const teachersSheet = XLSX.utils.aoa_to_sheet([
      ["اسم المعلم", "رقم الحساب", "رقم الهوية", "الحلقة", "المسمى"],
      ...teachers.map((teacher) => [
        teacher.name || "",
        teacher.accountNumber || "",
        teacher.idNumber || "",
        teacher.halaqah || "",
        teacher.role === "deputy_teacher" ? "deputy_teacher" : "teacher",
      ]),
    ])

    const circlesSheet = XLSX.utils.aoa_to_sheet([
      ["اسم الحلقة"],
    ...
    const rolesSheet = XLSX.utils.aoa_to_sheet([
      ["المسمى", "القيمة"],
      ["معلم", "teacher"],
      ["نائب معلم", "deputy_teacher"],
    ])

    XLSX.utils.book_append_sheet(workbook, templateSheet, "نموذج الإضافة")
    XLSX.utils.book_append_sheet(workbook, teachersSheet, "المعلمون")
    XLSX.utils.book_append_sheet(workbook, circlesSheet, "الحلقات")
    XLSX.utils.book_append_sheet(workbook, rolesSheet, "المسميات")
  ...
  return (
    <>
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className={addDialogView === "bulk" ? "flex h-[84vh] max-h-[84vh] !w-[calc(100vw-1.5rem)] !max-w-[1120px] flex-col rounded-2xl bg-white p-0 overflow-hidden sm:!w-[90vw]" : "max-w-md rounded-2xl bg-white p-0 overflow-hidden"} dir="rtl" style={{ zIndex: 110 }}>
          ...
          {addDialogView === "single" ? (
            ...
            <>
              <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6 lg:px-8">
                <div className="space-y-3 pr-1 pb-2">
                  {bulkTeachers.map((draft) => (
                    <div key={draft.id} className="rounded-2xl border border-[#3453a7]/20 bg-[#fcfdff] p-4 shadow-sm sm:p-5">
                      <div className="grid grid-cols-1 items-end gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-[minmax(220px,1.35fr)_repeat(5,minmax(112px,1fr))]">
                        <div className="space-y-1.5">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Select value={draft.selectedHalaqah} onValueChange={(value) => updateBulkTeacher(draft.id, { selectedHalaqah: value })}>
                            <SelectTrigger className="h-10 rounded-xl border-[#3453a7]/40 text-sm focus:border-[#3453a7]"><SelectValue placeholder="اختر الحلقة" /></SelectTrigger>
                            <SelectContent style={{ zIndex: 140 }}>
                              {circles.map((circle) => (
              ...
              <div className="shrink-0 border-t border-[#3453a7]/25 bg-white px-4 py-4 shadow-[0_-8px_24px_rgba(52,83,167,0.06)] sm:px-6 lg:px-8">
                <div className="mb-4 flex flex-col items-stretch gap-3 lg:flex-row lg:items-center lg:justify-between">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                    <button type="button" onClick={handleExportTeachersExcel} className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#3453a7]/40 bg-white px-4 py-2 text-sm font-semibold text-[#4f73d1] transition-colors hover:bg-[#3453a7]/10">
